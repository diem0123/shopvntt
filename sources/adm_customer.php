<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('adm_customer_list'); 
		
		if (!$common->checkpermission($_SESSION['ID_CONSULTANT'], $common->getFeature(3), "V")){
		...
		// setup for paging						
			...
			$field_name_other = array('USER_NAME','STATUS','CREATED_DATE','UPDATED_DATE');			
			$fet_box = & new Template('adm_customerlist_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			
			$condit = " AND c.ID_CONSULTANT = chr.ID_CONSULTANT AND chr.ROLE_ID = rlk.ROLE_ID AND rlk.ROLE_ID = '".$idrole."'";
			...
			{								
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("adm_customerlist_rs");
	
				}
								
				if($datasub != "")
				$main->set('customerlist_rs', $datasub);
				else{
				...
				// Paging
					$conditcount = " c.STATUS <> 'Deleted' AND c.ID_CONSULTANT = chr.ID_CONSULTANT AND chr.ROLE_ID = rlk.ROLE_ID AND rlk.ROLE_ID = '".$idrole."'";
					$tb = "consultant c, consultant_has_role chr, role_lookup rlk ";
					$totalpro = $common->countRecord($tb,$conditcount,"ID_CONSULTANT");
					
					$link_page = "index.php?act=adm_customer";
					$class_link = "do";
