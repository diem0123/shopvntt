<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('adm_brand_list');
	...
	$main->set('submenu2',"");				 
		...
		// setup for paging			
			...
			$field_name_other = array('NAME_BRAND','CONTENT','IORDER','STATUS');			
			$fet_box = & new Template('adm_brandlist_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " 1 ";
			$sql="SELECT * FROM brand  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
				
			while ($rs=$DB->fetch_row($query))
			{								                   				
					$stt++;
					$fet_box->set( "CATNAME", $rs['NAME_BRAND']);
					$fet_box->set( "idcat", $rs['ID_BRAND']);
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("adm_brandlist_rs");
	
				}
							
				if($datasub != "")
				$main->set('brandlist_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("brand",$condit,"ID_BRAND");
					
					$link_page = "index.php?act=adm_brand";
					$class_link = "linkpage";
