<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('subcatelement_list');
	...
	$main->set('submenu2',$submenu2);				 
		...
		// setup for paging			
			...
			$fet_box = & new Template('subcatelementlist_rs');
	...
	*/		
			...
			{								                   				
					$stt++;
					$fet_box->set( "name", $rs[$field_name]);
					...
					$fet_box->set("NAMECAT", $catinfo['NAME_CATEGORY']);
					
					// get name category
					$catinfo = $common->getInfo("categories","ID_CATEGORY = '".$rs['ID_CATEGORY']."'");
					$fet_box->set("NAMECAT", $catinfo['NAME_CATEGORY']);
					$typeinfo = $common->getInfo("categories_sub","ID_TYPE = '".$rs['ID_TYPE']."'");
					$fet_box->set("NAME_TYPE", $typeinfo['NAME_TYPE']);
					
					
					// Get info datasub
					$datasub .= $fet_box->fetch("subcatelementlist_rs");
	
				}
							
				if($datasub != "")
				$main->set('subcatelementlist_rs', $datasub);
				else{
				...
				// Paging
					...
					$link_page = "index.php?act=subcatelementlist&idtype=".$idtype;
					$class_link = "do";
