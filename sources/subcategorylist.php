<?php
	...
	$submenu->set('sub','');
     //main
	 if($idcat==7)
	 $main = & new Template('mt_list');
	 else
     $main = & new Template('subcategory_list');
	...
	$main->set('submenu2',$submenu2);				 
		...
		$main->set("title", $pagetitle);
			...
			$fet_box = & new Template('subcategorylist_rs_mt');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " 1 ";
			$sql="SELECT * FROM material_lookup  WHERE '".$condit."' ORDER BY IORDER ";    
			$query=$DB->query($sql); 										
			...
			{								                   				
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("subcategorylist_rs_mt");
	
				}
							
				if($datasub != "")
				$main->set('subcategorylist_rs', $datasub);
				else{
			...
			$field_name_other = array('IORDER','LINK','NAME_TYPE','ID_CATEGORY','STATUS');			
			$fet_box = & new Template('subcategorylist_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " ID_CATEGORY = '".$idcat."'";
			$sql="SELECT * FROM categories_sub  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
			...
			{								                   				
					...
					// get name category
					$catinfo = $common->getInfo("categories","ID_CATEGORY = '".$rs['ID_CATEGORY']."'");
					$fet_box->set("NAMECAT", $catinfo['NAME_CATEGORY'.$pre]);					
					
					// Get info datasub
					$datasub .= $fet_box->fetch("subcategorylist_rs");
				...
				// Paging
					$totalpro = $common->countRecord("categories_sub",$condit,"ID_TYPE");
					
					$link_page = "index.php?act=subcategorylist&idcat=".$idcat;
					$class_link = "linkpage";
