<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('category_list');
	...
	$main->set('submenu2',$submenu2);				 
		...
		// setup for paging			
			...
			$fet_box = & new Template('categorylist_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " 1 ";
			$sql="SELECT * FROM categories  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
			...
			{								                   				
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("categorylist_rs");
	
				}
							
				if($datasub != "")
				$main->set('categorylist_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("categories",$condit,"ID_CATEGORY");
					
					$link_page = "index.php?act=adm_category";
					$class_link = "linkpage";
