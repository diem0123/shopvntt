<?php
	...
	$main->set('submenu2',"");				 
		...
		// setup for paging			
			...
			$field_name_other = array('IORDER','NAME','CODE','ID_PROVINCE');			
			$fet_box = & new Template('distlist_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " ID_PROVINCE = '".$idpr."'";
			$sql="SELECT * FROM district  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
				
			while ($rs=$DB->fetch_row($query))
			{								                   				
					$stt++;
					$fet_box->set( "NAME", $rs['NAME']);
					$fet_box->set( "ID_DIST", $rs['ID_DIST']);
					$fet_box->set( "ID_PROVINCE", $rs['ID_PROVINCE']);
					$fet_box->set( "stt", $stt);
					...
					// get name category
					$catinfo = $common->getInfo("province","ID_PROVINCE = '".$rs['ID_PROVINCE']."'");
					$fet_box->set("namepr", $catinfo['NAME']);
					
					// Get info datasub
					$datasub .= $fet_box->fetch("distlist_rs");
	
				}
							
				if($datasub != "")
				$main->set('distlist_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("district",$condit,"ID_DIST");
					
					$link_page = "index.php?act=adm_dist&idpr=".$idpr;
					$class_link = "linkpage";
