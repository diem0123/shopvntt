<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('adm_capacity_list');
	...
	$main->set('submenu2',"");				 
		...
		// setup for paging			
			...
			$field_name_other = array('NAME_CAPACITY','CONTENT','IORDER','STATUS','ID_PRODUCT');			
			$fet_box = & new Template('adm_capacity_list_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " 1 AND STATUS = 'Active' ";
			$sql="SELECT * FROM capacity_product  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
				
			while ($rs=$DB->fetch_row($query))
			{	
					$stt++;
					$fet_box->set( "CATNAME", $rs['NAME_CAPACITY']);
					$fet_box->set( "idcapacity", $rs['ID_CAPACITY']);
					$fet_box->set( "content", $rs['CONTENT']);
					$fet_box->set( "ID_PRODUCT", $rs['ID_PRODUCT']);
					$fet_box->set( "tinhtrang",$rs['TINHTRANG']);
					$fet_box->set( "stt", $stt);
				...
				$productinfo = $common->getInfo("products","ID_PRODUCT = ".$rs['ID_PRODUCT']);
				$fet_box->set("PRODUCT_NAME", $productinfo['PRODUCT_NAME']);
					// Get info datasub
					$datasub .= $fet_box->fetch("adm_capacity_list_rs");
				}		
				if($datasub != "")
				$main->set('capacity_list_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("capacity_product",$condit,"ID_CAPACITY");
					
					$link_page = "index.php?act=adm_capacity";
					$class_link = "linkpage";
