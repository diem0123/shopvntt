...
$DB->query( "UPDATE common SET COUNTVIEW = COUNTVIEW+1 WHERE ID_COMMON = '".$idn."'" );
	...
	//get detail
$condit = " ID_COMMON = '".$idn."' AND STATUS = 'Active' AND STITLE".$pre." <> '' ";		
$info_news = $common->getInfo("common",$condit);
if(isset($info_news['ID_TYPE'])){
...
//Get name category
	$data_cate = $common->getInfo("common_cat","ID_TYPE = ".$get_idtype." AND ID_CAT = ".$get_idcat);
	$name_cat = $data_cate['SNAME'];
// get menuselect
	// get menuselect
	if($get_idtype ==19){
		if($get_idcat ==102)$_SESSION['tab'] =5;			
		elseif($get_idcat==103)$_SESSION['tab'] =4;
		elseif($get_idcat==104)$_SESSION['tab'] =6;		
	}else{
		$menuinfo = $common->getInfo("menu1","IDMUC = '".$get_idtype."'");
		if(isset($menuinfo['IDMUC'])) $_SESSION['tab'] = $menuinfo['IORDER'];
		else $_SESSION['tab'] = 1;
	}

	
	//echo $menuinfo['IORDER'];exit;
...
$field_name_other2_news = array("ID_CAT","IORDER");
	...
	$condit_other = "";
if(isset($info_news['ID_CAT_SUB']) && $info_news['ID_CAT_SUB'] > 0){
...
elseif(isset($info_news['ID_CAT']) && $info_news['ID_CAT'] > 0){
...
if($get_idtype == 10 || $get_idtype == 11){
$addmore = " AND STITLE".$pre." <> '' ";
$condit_other .= $addmore;
}

$field_name_other = array("SCONTENTSHORT","SCONTENTS","ID_TYPE","PICTURE","ID_CAT","ID_CAT_SUB");
if($get_idcat !=106)
	$list_news_rs = $print_2->list_row_multi_field("common", $condit_other, "ID_COMMON", "STITLE", $field_name_other, " ORDER BY DATE_UPDATED DESC", "detail1_list_other", 0,6, 1, $idmnselected=0, $pre );
else
	$list_news_rs = $print_2->list_row_multi_field("common", $condit_other, "ID_COMMON", "STITLE", $field_name_other, " ORDER BY DATE_UPDATED DESC", "detail1_list_video_other", 0,6, 1, $idmnselected=0, $pre );
...
// TIn tức liên quan
$main_info->set('list_news_other', $list_news_rs);
	// get title	
...
$main_info->set('list_qc_top_right1',$list_qc_top_right1);
...
$main_info->set('list_qc_top_right2',$list_qc_top_right2);
...
$main_info->set('list_sp_new_right',$list_sp_new_right);
