...
//	mysqli_query("SET character_set_results=utf8");
    if (!isset($_SESSION['ID_CONSULTANT']) || $_SESSION['ID_CONSULTANT'] == "") {	 	
     ...
     //main
     $main = & new Template('adm_importexcel_cap'); 	 
	...
	if($idcom && $ac=="a" && $submit == ""){
		 echo $main->fetch("adm_importexcel_cap");
				...
				$data = $uploader->upload($_FILES['files'], array(
					...
					foreach($info['metas'] as $getdata){
						...
						for($i=0;$i<$num;$i++){
							...
							if(!empty($get_array[$i][12])){$id_pro_cat = $get_array[$i][12];}else {	$id_pro_cat = 0;}
							if($rows_check==''){
...
/*	
	if($ac=='a'& $ex!=''){
		$conn=mysqli_connect("localhost", "root", "root","lacoco_db") or die("can't connect database");
		$ex_sql = 'select * from products where ID_PRODUCT=46';
		mysql_query("SET NAMES 'UTF8'");
		$result = mysqli_query($conn,$ex_sql);
