<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('provinces_list');
	...
	$main->set('submenu2',"");				 
		...
		// setup for paging			
			...
			$field_name_other = array('IORDER','NAME','CODE');			
			$fet_box = & new Template('provinceslist_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " 1 ";
			$sql="SELECT * FROM province  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
			...
			{								                   				
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("provinceslist_rs");
	
				}
							
				if($datasub != "")
				$main->set('provinceslist_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("province",$condit,"ID_PROVINCE");
					
					$link_page = "index.php?act=adm_provinces";
					$class_link = "linkpage";
