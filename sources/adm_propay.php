<?php
	...
	$main->set('submenu2',"");				 
		...
		// setup for paging			
			...
			$field_name_other = array('PROPAY_NAME','IORDER','EMAIL_PAY','MERCHANT','ACCESSCODE');			
			$fet_box = & new Template('propaylist_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " 1 ";
			$sql="SELECT * FROM payments_lookup  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
				
			while ($rs=$DB->fetch_row($query))
			{								                   				
					$stt++;
					$fet_box->set( "propayname", $rs['PROPAY_NAME']);
					$fet_box->set( "idpropay", $rs['ID_PROPAY']);
					$fet_box->set( "emailpay", $rs['EMAIL_PAY']);
					$fet_box->set( "merchant", $rs['MERCHANT']);
					$fet_box->set( "accesscode", $rs['ACCESSCODE']);
					$fet_box->set( "stt", $stt);
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("propaylist_rs");
	
				}
							
				if($datasub != "")
				$main->set('propaylist_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("payments_lookup",$condit,"ID_PROPAY");
					
					$link_page = "index.php?act=adm_propay";
					$class_link = "do";
