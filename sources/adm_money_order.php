<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('adm_moneyorder_list');
	...
	$main->set('submenu2',$submenu2);				 
		...
		// setup for paging			
			...
			$field_name_other = array('NAME','FROM_MONEY','TO_MONEY','IORDER','ID_MONEY_TRANS','TYPE_TRIGIA');			
			$fet_box = & new Template('adm_moneyorder_list_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " 1 ";
			$sql="SELECT * FROM money_transfer_lookup  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
				
			while ($rs=$DB->fetch_row($query))
			{								                   				
					$stt++;
					$fet_box->set( "namemn", $rs['NAME']);
					$fet_box->set( "idmn", $rs['ID_MONEY_TRANS']);					
					$fet_box->set( "stt", $stt);
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("adm_moneyorder_list_rs");
	
				}
							
				if($datasub != "")
				$main->set('moneyorder_list_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("money_transfer_lookup",$condit,"ID_MONEY_TRANS");
					
					$link_page = "index.php?act=adm_money_order";
					$class_link = "do";
