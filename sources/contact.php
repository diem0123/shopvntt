...
// kiem tra logged in
	...
	//Get form nhận email khuyến mãi
	$res_submit = $itech->input['res_email'];
	$nt_content = $itech->input['nt_email'];
	
	//End get form nhận email khuyến mãi
	// get menuselect
	...
	if(isset($res_submit) && $res_submit != ""){
		$subject="Email";
		$tplcontent = new Template('tpl_contact');
		$tplcontent->set('email', $nt_content);
		$tplcontent->set('content', $nt_content);
		$nt_content= $tplcontent->fetch('tpl_contact');
		$ids = 1;
		$getsmtp = $common->getInfo("outgoing_email","OUTGOING_EMAIL_ID = '".$ids."'");
		
		$emailfrom = $nt_content;					
		$namefrom = 'Email đăng ký';//$fullname; email dem gui thay
		$emailto = $getsmtp['FROM_EMAIL'];//$itech->vars['email_webmaster'];
		//$emailto = "thanle76@gmail.com";
		$nameto = $getsmtp['FROM_NAME'];//$INFO['name_webmaster'];
		$mail = new emailtemp();
		$error = $mail->sendmark($emailfrom,$namefrom,$emailto,$nameto,$nt_content,$subject);					
		// redirect to register success
		if($error == "ok") {
			$_SESSION['mess_forgot1'] = $lang['tl_thongtinsent'];
			$_SESSION['mess_forgot2'] = "";
