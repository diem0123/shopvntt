<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('adm_hotnews_list'); 
		...
		// setup for paging			
			...
			$field_name_other = array('STITLE','ID_COMMON','IORDER','STATUS');			
			$fet_box = & new Template('adm_hotnews_list_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$condit = " STATUS <> 'Deleted' ";
			$sql="SELECT * FROM hotnews WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
			
			while ($rs=$DB->fetch_row($query))
			{								                   
					$fet_box->set( "STITLE", $rs['STITLE']);
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("adm_hotnews_list_rs");
				...
				// Paging					
					$totalpro = $common->countRecord("hotnews",$condit,"IDHOT");
					
					$link_page = "index.php?act=adm_hotnews";
					$class_link = "do";
