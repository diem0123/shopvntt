<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('adm_commontype_list');
	...
	$main->set('submenu2',$submenu2);				 
		...
		// setup for paging			
			...
			$fet_box =  new Template('adm_commontype_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			
			//$condit = " STATUS = 'Active' ";
			$condit = " STATUS <> 'Deleted' ";
			$sql="SELECT * FROM common_type  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
			...
			{		
				if($rs['SNAME'.$pre]!=''){
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("adm_commontype_rs");
	
				}
								
				if($datasub != "")
				$main->set('commontype_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("common_type",$condit,"ID_TYPE");
					
					$link_page = "index.php?act=adm_common";
					$class_link = "do";
