<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('adm_contacts_list');
	...
	$main->set('submenu2',"");				 
		...
		// setup for paging			
			...
			$field_name_other = array('CREATED_DATE','UPDATED_DATE','NEWSLETTER', 'SEMAIL', 'SFULLNAME');			
			$fet_box = & new Template('adm_contacts_list_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " SEMAIL <> '' ";
			$sql="SELECT * FROM contact  WHERE ".$condit." ORDER BY UPDATED_DATE DESC LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
				
			while ($rs=$DB->fetch_row($query))
			{								                   				
					$stt++;
					$fet_box->set( "SFULLNAME", $rs['SFULLNAME']);
					$fet_box->set( "IID", $rs['IID']);
					$fet_box->set( "stt", $stt);
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("adm_contacts_list_rs");
	
				}
							
				if($datasub != "")
				$main->set('contacts_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("contact",$condit,"IID");
					
					$link_page = "index.php?act=adm_contacts";
					$class_link = "do";
