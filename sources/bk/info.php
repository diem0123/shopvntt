<?php
	...
	if($display_list ==1){		
		...
		elseif($idtype == 10 || $idtype == 5  ||$idtype == 9 || $idtype == 11 ||$idtype ==7 || $idtype==4 ){
			$list_news_rs = $print_2->list_row_column_multifield("common", $condit, "ID_COMMON", "STITLE", $field_name_other, " ORDER BY DATE_UPDATED DESC, ID_COMMON DESC ", "list_3news_rs", 3, $pageoffset, $num_record_on_page, $page, $idmnselected, $pre );
		...
		if($_SESSION['dv_Type']=="phone"){
			if($idcat==11) $tplrs = "m_list_recruit_rs";
			else $tplrs = "m_list_news_rs";
			$tplnews = "m_list_news";
		}
		else{
			if($idcat==11) $tplrs = "list_recruit_rs";
			else $tplrs = "list_search_rs";//"list_3news_rs";
