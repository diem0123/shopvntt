<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('adm_pricesearch_list');
	/*
...
*/			
	$main->set('submenu2',"");				 
		...
		// setup for paging			
			...
			$field_name_other = array('NAME','FROM_MONEY','TO_MONEY','IORDER','ID_PRICE');			
			$fet_box = & new Template('adm_pricesearch_list_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " 1 ";
			$sql="SELECT * FROM price_lookup  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
			...
			{								                   				
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("adm_pricesearch_list_rs");
	
				}
							
				if($datasub != "")
				$main->set('pricesearch_list_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("price_lookup",$condit,"ID_PRICE");
					
					$link_page = "index.php?act=adm_price_search";
					$class_link = "do";
