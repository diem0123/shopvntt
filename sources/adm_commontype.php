<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('adm_commoncat_list');
	...
	$main->set('submenu2',$submenu2);				 
		...
		// setup for paging			
			...
			$fet_box = & new Template('adm_commoncat_list_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			
			//$condit = " STATUS = 'Active' ";
			$condit = " STATUS <> 'Deleted' AND ID_TYPE = '".$idtype."'";
			$sql="SELECT * FROM common_cat  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
			...
			{								
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("adm_commoncat_list_rs");
	
				}
								
				if($datasub != "")
				$main->set('commoncat_list_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("common_cat",$condit,"ID_CAT");
					
					$link_page = "index.php?act=adm_commontype";
					$class_link = "do";
