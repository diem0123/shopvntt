<?php
	...
	$submenu->set('sub','');
     //main
     $main = & new Template('adm_material_list');
	...
	$main->set('submenu2',"");				 
		...
		// setup for paging			
			...
			$field_name_other = array('NAME','IORDER','STATUS');			
			$fet_box = & new Template('adm_materiallist_rs');
			$datasub = ""; // de lay du lieu noi cac dong
			$stt =0;
			
			$condit = " 1 ";
			$sql="SELECT * FROM material_lookup  WHERE ".$condit." ORDER BY IORDER LIMIT ".$pageoffset.",".$num_record_on_page;    
			$query=$DB->query($sql); 										
				
			while ($rs=$DB->fetch_row($query))
			{								                   				
					$stt++;
					$fet_box->set( "CATNAME", $rs['NAME']);
					$fet_box->set( "idcat", $rs['ID_MT']);
					$fet_box->set( "stt", $stt);
					...
					// Get info datasub
					$datasub .= $fet_box->fetch("adm_materiallist_rs");
	
				}
							
				if($datasub != "")
				$main->set('materiallist_rs', $datasub);
				else{
				...
				// Paging
					$totalpro = $common->countRecord("brand_lookup",$condit,"ID_BR");
					
					$link_page = "index.php?act=adm_material";
					$class_link = "linkpage";
