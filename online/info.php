<?php
	...
	$data_cat = $common->getInfo('common_cat',"ID_TYPE =".$idtype." AND ID_CAT =".$idcat);
	if($data_cat['SNAME'.$pre] =='')
		$name_cat = $data_cat['SNAME'];
	else
		$name_cat = $data_cat['SNAME'.$pre];
	...
	$num_record_on_page = 8;//$itech->vars['num_record_on_page'];
...
*/
	...
	$header = $common->getHeader('m_header');
		...
		/*
		if($idtype==1)
			$header = $common->getHeader('header_bk',$idtype);
		else*/
	...
	if($display_list ==1){		
		...
		elseif($idtype == 10 || $idtype ==15){
			
			$condit_type = " ID_TYPE = '".$idtype."' AND STATUS = 'Active' ";
			$info_news = $common->getInfo("common_type",$condit_type);
			$main_info->set('name_type',$info_news['SNAME'.$pre]);
		...
		if($idtype ==2 and $idcat ==''){
			$content_decode = $print->get_data_cat('common_cat',2,$pre,'list_info_dv','list_dichvu_rs');
		}
		else{
	...
	//Hỗ trợ trực tuyến
	$support = $common->gethonhop(177);
	if($support == "")
	$main_info->set('support', "");
	else{
		//$tpl->set('title_welcome', $welcome['STITLE'.$pre]);
	if($support['SCONTENTS'.$pre]=="")
		$main_info->set('support', base64_decode($support['SCONTENTS']));		
	else
		$main_info->set('support', base64_decode($support['SCONTENTS'.$pre]));
		//echo (base64_decode($welcome['SCONTENTS'.$pre]));	
	}
	
	//Like fan Page
	$fanpage = $common->gethonhop(182);
	if($fanpage == "")
	$main_info->set('fanpage', "");
	else{
		//$tpl->set('title_welcome', $welcome['STITLE'.$pre]);
	if($fanpage['SCONTENTS'.$pre]=="")
		$main_info->set('fanpage', base64_decode($fanpage['SCONTENTS']));		
	else
		$main_info->set('fanpage', base64_decode($fanpage['SCONTENTS'.$pre]));
		//echo (base64_decode($welcome['SCONTENTS'.$pre]));	
	}
	...
	$list_hoatdong = $print->get_data_filter('common',10,$pre,0,'list_hdong_ttich','list_hdong_ttich_rs','','');
	$main_info->set('list_img_hoatdong',$list_hoatdong);

	// Danh sách tin xem nhiều nhất
	$list_news_views = $print->get_data_viewcount('common',$pre,'list_news_views','list_news_views_rs');
	$main_info->set('list_news_views',$list_news_views);
	...
	//.............Set danh sách bài viết được xem nhiều nhất .............................
	$listviewcount = $print->get_data_viewcount('common',$pre,'list_countview','list_countview_rs');
	//print_r($listviewcount);exit;
	$main_info->set('list_viewcount',$listviewcount);
	// .............................................................
	...
	// get cong ty footer
	//$menucs = $print->get_data('common',10,$pre,1,'list_menucs','list_menucs_rs');
	
	//$footer->set('menucs', $menucs);
	...
	// Coppy @ right
	$cop = $common->gethonhop(169);
	if($cop == "")
		$footer->set('cop', "");
	else{		
		if($cop['SCONTENTS']=="")
			$footer->set('cop', base64_decode($cop['SCONTENTS']));
		else
			$footer->set('cop', base64_decode($cop['SCONTENTS'.$pre]));
	}
	// get support
	$address = $common->gethonhop(168);
	if($address == "")
		$footer->set('address', "");
	else{		
		if($address['SCONTENTS']=="")
			$footer->set('address', base64_decode($address['SCONTENTS']));
		else
			$footer->set('address', base64_decode($address['SCONTENTS'.$pre]));
	}
	$menudv = $print->get_data_cat('common_cat',2,$pre,'list_menucs','list_menucs_rs');
	
	$footer->set('menudv', $menudv);
	// Link Face + Google ...
	$linkf = $common->gethonhop(170);
	if($cop == "")
		$footer->set('linkf', "");
	else{		
		if($linkf['SCONTENTS']=="")
