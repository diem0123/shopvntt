...
function savenumcart(idp,idcap,od){
			var idview = '#'+ idp;
			...
			$.ajax({	
				...
				data:'idp='+idp+'&od='+od+'&idcap='+idcap,
				type: 'POST',
				cache: false,
				beforeSend: function(){
			     	$("#loading").show();
...
function savenumcart_pro(idp,od,numql){
			var idview = '#'+ idp;
			//var seconds = new Date().getSeconds();
			//$("#base_url").val()+	
			
			$.ajax({	
				url: 'index.php?act=savecartpro_pro',
				data:'idp='+idp+'&od='+od+'&numql='+numql,
				type: 'POST',
				cache: false,
				beforeSend: function(){
			     	$("#loading").show();
...
function savecart(idp,od){
	...
	function selemenu(stt) {		
		$.ajax({
				url: 'index.php?act=setmenu',
				type: 'POST',
				data: 'stt='+stt,
				success: function(data) {
				//alert('123');
