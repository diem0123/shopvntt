		...
		for($kd=1;$kd<=$num_element;$kd++){
			...
			if($kd>1){
				...
				/*
					...
					$get_namecat = $common->getInfo('product','ID_COMMON = '.$rs[$field_id].' AND STATUS = "Active"');
					$get_idtype = $get_namecat['ID_TYPE'];
					$get_namecatt = $common->getInfo('categories_sub','ID_TYPE = '.$get_idtype.' AND STATUS = "Active"');
					$name_link_cat = $this->vn_str_filter($get_namecatt['NAME_TYPE']);
					$tpl->set('name_link_cat'.$p,$name_link_cat);
					$tpl->set('ID_TYPE'.$p,$get_namecatt['ID_TYPE']);
					$tpl->set('ID_CATEGORY'.$p,$get_namecatt['ID_CATEGORY']);
				
				 */
					// Set name 
					 
                     $tpl->set( "name".$p, $resultname );
					// $tpl->set( "name".$p, $this->cleanchar( $resultname, 60, false) );
					...
					if(isset($rs['IMAGE_LARGE'])){
						...
						if($tb_name == "products") {
							...
							$tpl->set( "w".$p, $wh[0]);
							$tpl->set( "h".$p, $wh[1]);
							$contentshort = $this->cleanchar( $rs['INFO_SHORT'.$pre], 100, false );
					...
					elseif(isset($rs['PICTURE'])){
						...
						elseif($tb_name == "common") {
							...
							$wh = $this->changesizeimage($rs['PICTURE'], $origfolder, $width, $height);
							 $tpl->set( "w".$p, $wh[0]);
							 $tpl->set( "h".$p, $wh[1]);
							 if($rs['SCONTENTSHORT'.$pre]!='')
								$contentshort = $this->cleanchar( $rs['SCONTENTSHORT'.$pre], 120, false );
							 else
								 $contentshort = $this->cleanchar( $rs['SCONTENTSHORT'], 120, false );
							 $tpl->set( "contentshort".$p, $contentshort); 
						}
					}
					...
					if(isset($rs['PICTURE'])){
						...
						if($tb_name == "common") {
							...
							$wh = $this->changesizeimage($rs['PICTURE'], $origfolder, $width, $height);
							 ...
							 // danh rieng cho search
							 if($temp_chid == "list_search_rs"){
							 $contentshort = $this->cleanchar( $rs['SCONTENTSHORT'.$pre], 260, false );
							 $tpl->set( "contentshort", $contentshort);
							 }
						}
					}
