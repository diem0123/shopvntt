			...
			if($kd>1){
				...
				// get name 
				 ...
				 //echo $resultname."<br>";
					...
					// Set name 
					 ...
					 $tpl->set('name_cat'.$p,$_SESSION['get_title']);
					 $sql_cat=$common->getInfo('common','ID_COMMON ='.$rs[$field_id]);
					 $id_catt=$sql_cat['ID_CAT'];
					 $id_typee=$sql_cat['ID_TYPE'];
					 $sql_cat1=$common->getInfo('common_cat','ID_CAT='.$id_catt);
					 $tpl->set('name_cat'.$p,$sql_cat1['SNAME']);
					$name_link_cat = $this->vn_str_filter($sql_cat1['SNAME']);

					 $tpl->set('getidtype'.$p,$id_typee);
					 $tpl->set('getidcat'.$p,$id_catt);
					 $tpl->set('name_link_cat'.$p,$name_link_cat);
					...
					if(isset($rs['THUMBNAIL'])){
						...
						if($tb_name == "products") {
							...
							$wh = $this->changesizeimage($rs['THUMBNAIL'], $origfolder, $width, $height);
							 $tpl->set( "wth".$p, $wh[0]);
							 $tpl->set( "hth".$p, $wh[1]);
							 $tpl->set( "THUMBNAIL".$p, $rs['THUMBNAIL']);
						}
					}
				...
				*/
                  ...
                  if($rs['SALE'] != "" && $rs['SALE']>0){
                     ...
                     $tpl->set("SALE", $rs['SALE']);
                     $tpl->set("PRICE_DISC", number_format($rs['PRICE']*$rs['SALE']/100,0,"","."));
                 }						                 
                 else{
                     $tpl->set("PRICE_SALE", "");
                     $tpl->set("SALE", "");
...
public function list_row_multi_field($tb_name, $condition, $field_id, $field_name, $field_name_other, $iorder, $temp_chid, $from=0,$to=0, $page=0, $idmnselected=0, $pre )
 ...
 function pagingphp($TotalRows,$RowsInPage,$maxpage, $url, $page, $class_link){
	...
	if ($maxpage) $numfrom = 1;
		...
		if ($page>1)
		$str = "<li class='".$class_link."' class='preicon'><a href='$url&page=".($page-1)."'  > &lt; </a></li>";
		else
		$str="";
		//Middle
		if ($end<$numpage){
			...
			for($i=$start;$i<=$end;$i++){
				if ($k==0)
				{
					...
					$str.="<li class='".$class_link."' class='numpaging'><a href='$url&page=".$i."' >".$i."</a></li>";
				}
				else{
					if ($i==$page)
		...
		if ($page<$numpage)
		{
			$str.="<li class='".$class_link."' class='nexticon'><a href='$url&page=".($page+1)."' > &gt; </a></li>";
		}
		else
		$str.="";
		return $str;
	...
	if ($maxpage) $numfrom = 1;
		...
		if ($page>1)
		$str="<li><a href='$url&page=".($page-1)."'  > &laquo; </a></li>";
		else
		$str="";
		//Middle
		if ($end<$numpage){
			...
			for($i=$start;$i<=$end;$i++){
				if ($k==0)
				{
					...
					$str.="<li class='".$class_link."'><a href='$url&page=".$i."' >".$i."</a></li>";
		...
		if ($page<$numpage)
		{
			$str.="<li><a href='$url&page=".($page+1)."' > &raquo; </a></li>";
		}
		else
		$str.="";
		return $str;
	...
	function copy_and_change_filename_dd( $file_input_tmp, $file_input_name, $dir_upload, $prefix )
    {
        global $print;
       ...
       // $part = explode( ".", $file_input_name );
		...
		//xu ly file anh ban quyen
		 $result = $print->create_watermark($dest, $dest);
		if ($result === false) {
			if($cp) return $file_name;
			else return "";
		} else {
			return $file_name;
		}
	...
	global $DB, $lang, $itech, $std, $common, $print_2,$print;					
			...
			if(!isset($_SESSION['shoppingcart']) || $_SESSION['shoppingcart'] == "") $list_idp = "''";
				...
				if($idcom > 0){
					...
					//+++++++++++++++++++ check mobile +++++++++++++++++++++	
					if($_SESSION['dv_Type']=="phone" || $_SESSION['dv_Type_giohang']=="phone") $tplod = "m_order_viewcart_list_rs";
					...
					{												
						...
						// use get list for send mail order list products
							...
							//+++++++++++++++++++ check mobile +++++++++++++++++++++	
							if($_SESSION['dv_Type']=="phone" || $_SESSION['dv_Type_giohang']=="phone") {
	...
	function GetListMN_right($field_name_other1, $temp1,  $field_name_other2="", $temp2="", $field_name_other3="", $temp3="", $pre, $idc="", $idt="", $idprocat="", $ida="", $idsk="")
{
 ...
 public function getstar($idp) {
	...
	function GetListMN_header($field_name_other1, $temp1,  $field_name_other2="", $temp2="", $field_name_other3="", $temp3="", $pre, $idc="", $idt="", $idprocat="", $ida="", $idsk="")
{
	global $DB, $lang, $itech, $std,$print;
	$sql1="SELECT * FROM common_type WHERE STATUS = 'Active' AND ID_TYPE = '".$idc."' ORDER BY IORDER";	
	$query1=$DB->query($sql1);
