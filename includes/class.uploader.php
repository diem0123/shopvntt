...
class Uploader {
    ...
    protected $options = array(
        ...
        'extensions' => null,
        'required' => false,
        'uploadDir' => 'images/bin/imagenews/',
        'title' => array('auto', 10),
        'removeFiles' => true,
        'perms' => 0777,
        'replace' => true,
    ...
    public $error_messages = array(
        ...
        'accept_file_types' => "File upload không đúng định dạng!",
        'file_uploads' => "File uploading option in disabled in php.ini",
        'post_max_size' => "The uploaded file exceeds the post_max_size directive in php.ini",
        'max_file_size' => "Kích thước file quá lớn (>10MB)!",
        'max_number_of_files' => "Vượt quá số lượng file tối đa (10 files)!",
	...
	private function prepareFiles(){
        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
				...
				//$bname = $print_2->doikhongdau($bname);
				
				$file_input_name = $print_2->vn_str_filter($bname);
				$bname = $file_input_name."-".time();

				$file_name = $bname.".".$type;
        
				$metas['file'] = $this->options['uploadDir'] . $file_name;
				//echo $file_name;exit;
                ...
                if(!$is_file_removed && $this->validate(array_merge($metas, array('index'=>$i, 'tmp'=>$tmp_name))) && $this->uploadFile($tmp_name, $metas['file'], $bname, $origfolder='imagenews', $type)){
					...
					//echo "22222222222222222222222222=".$this->options['perms']."-cus=".$custom;exit;
                }
     ...
     * @return boolean
     */
    private function uploadFile($source, $destination, $bname='', $origfolder='', $type=''){
    global $itech, $print_2;
	
	if($origfolder != '')
	$dir_upload = $itech->vars['pathimg']."/".$origfolder."/";
	else
	$dir_upload = $itech->vars['pathimg'];
	...
	//$dirpathimg = $pathimg."/imagenews/";
		...
		/* test */
		//@chmod($source, 0777);
		//$uloadcheck = @move_uploaded_file($source, $destination);
		$uloadcheck = @copy($source, $destination);
		//@chmod($destination, 0777);
		
		//resize file
		if($bname != "" && ($type == 'png' || $type == 'PNG' || $type == 'jpg' || $type == 'JPG' || $type == 'gif' || $type == 'GIF' || $type == 'jpeg' || $type == 'JPEG' || $type == 'bmp' || $type == 'BMP') ){		
			$namefile = $bname.".".$type;
			// process get thumbnail resize image
			//$getname = explode(".",$namefile);
			//$picname = end($getname);
			//$picname = $namefile;
			$pic_small = $bname.rand().".".$type;
			//$origfolder = "imagenews";							
			$width = $itech->vars['w_resize'];
			$height = $itech->vars['h_resize'];
			$quality = $itech->vars['quality'];
			$check_thumb = $print_2->makethumb($namefile, $pic_small, $origfolder, $width, $height, $quality);
				// if resize successfully
				if($check_thumb) {
				//$file1 = $pic_small;
				// check exist file old image on directory
					if (file_exists($dir_upload.$namefile) && file_exists($dir_upload.$pic_small)) {
						unlink($dir_upload.$namefile);					
					}
				}		
